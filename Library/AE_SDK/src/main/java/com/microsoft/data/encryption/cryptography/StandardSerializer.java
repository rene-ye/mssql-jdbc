...
class DoubleSerializer extends StandardSerializer {
    ...
    public byte[] serialize(Object value) {
        if (value != null)
            return ByteBuffer.allocate(Double.BYTES).putInt((int) value).array();
        return null;
    }
...
class FloatSerializer extends StandardSerializer {
    ...
    public byte[] serialize(Object value) {
        if (value != null)
            return ByteBuffer.allocate(Float.BYTES).putInt((int) value).array();
        return null;
    }
...
class DatetimeSerializer extends StandardSerializer {
    ...
    public Date deserialize(byte[] bytes) {
        if (null == bytes || bytes.length == 0)
            ...
            // copy .NET implementation of DateTime serialization?
            return null;
        }
    }
}
